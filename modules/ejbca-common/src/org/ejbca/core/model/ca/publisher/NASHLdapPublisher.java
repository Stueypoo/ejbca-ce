...
/**
 * NASHLdapPublisher is based upon the standard LdapPublisher, but is designed to update additional attributes in the LDAP entry
 * which are used by by the website for searching.
 * 
 ...
 *   validcpoids=<Certificate Policy OIDs representing a NASH certificate. Use ';' to separate entries.>
 ...
 *
 */
public class NASHLdapPublisher extends LdapPublisher implements ICustomPublisher{
	...
	public NASHLdapPublisher(){
		...
		// Set parameters to be used by this Publisher, and/or the super class 'LdapPublisher'.
		...
		setAddNonExistingAttributes(true);    // Add attrributes if they do not exist already in a LDAP entry.
		...
		setRemoveRevokedCertificates(true);   // Lets remove revoked certs. CleanDirectory application may also do this.
		setRemoveUsersWhenCertRevoked(true); // Lets leave the User entry as they may have other active certificates.
	 ...
	 */
	protected LDAPAttributeSet getAttributeSet(Certificate cert, String objectclass, String dn, String cpOid) {
		if (log.isTraceEnabled()) {
        ...
        // Will need to get 'cn' and 'dc' from DN.
		String[] attributesFromDN    = {"cn", "dc"}; 
        attributeSet.addAll(getAttributesFromDN(dn,  attributesFromDN));
        
        // Set the 'sn'
        attributeSet.add(new LDAPAttribute("sn", "NA"));
        ...
        if (cpOid.length() > 0) {
            attributeSet.add(new LDAPAttribute("employeeType", cpOid));
        }
	 ...
	 *
	 * @param oldEntry the existing LDAP entry
	 * @param dn dn of the LDAP entry.
	 * @param cpOid The text format of the CP OID value, or a blank string.
	 * @param cert the Certificate we are publishing, or null
	 *
	 * @return List of LDAPModification created...
	 */
	protected ArrayList<LDAPModification> getModificationSet(LDAPEntry oldEntry, String dn, String cpOid, Certificate cert) {
		if (log.isTraceEnabled()) {
			log.trace(">getModificationSet(dn="+ LogRedactionUtils.getSubjectDnLogSafe(dn)  + ")"); // email comes from SubjectDN and needs redaction
		}
...
//		final String objectclass = getUserObjectClass();
        ...
        // EmployeeType is the CP OID value
        if (cpOid.length() > 0) {
            LDAPAttribute attr = new LDAPAttribute("employeeType", cpOid);
